...
const STATUS_META: Record<RestaurantLead['status'], { label: string; bg: string; color: string }> = {
  pending:    { label: 'Pendiente',   bg: '#FFF7E0', color: '#B45309' },
  contacted:  { label: 'Contactado',  bg: '#EFF6FF', color: '#1D4ED8' },
...
function LeadCard({ lead, onStatusChange }: { lead: RestaurantLead; onStatusChange: (id: string, status: RestaurantLead['status']) => void }) {
  ...
  return (
    <div style={{
      background: '#fff', borderRadius: 14, border: '1px solid #F0E8E1',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
            <span style={{ fontFamily: sg, fontWeight: 700, fontSize: 16, color: '#1B1512' }}>
              {lead.nombreNegocio}
            </span>
            {lead.tipoNegocio && (
              <span style={{
          ...
          <div style={{ fontFamily: sg, fontSize: 13, color: '#5a4f47' }}>
            {lead.nombre}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 20px' }}>
        <a href={`https://wa.me/${lead.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: sm, fontSize: 13, color: '#1B1512', textDecoration: 'none' }}>
        ...
        {lead.email && (
          <a href={`mailto:${lead.email}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: sg, fontSize: 13, color: '#5a4f47', textDecoration: 'none' }}>
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#a89e95" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {lead.instagram && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: sg, fontSize: 13, color: '#5a4f47' }}>
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#a89e95" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="0.5" fill="#a89e95"/>
            </svg>
            @{lead.instagram}
          </span>
        )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontFamily: sg, fontSize: 12, color: '#9a8f86' }}>
        ...
        <span style={{ fontFamily: sm, fontSize: 11, color: '#b5a99f' }}>
      ...
      {lead.mensaje && (
        <div>
          ...
          {expanded && (
            <p style={{
              fontFamily: sg, fontSize: 13, color: '#5a4f47', lineHeight: 1.6,
              margin: '8px 0 0', background: '#FBF8F5', borderRadius: 8, padding: '10px 14px',
...
export function LeadsManager() {
  ...
  if (isLoading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 240, fontFamily: sg, color: '#9a8f86' }}>
    ...
    <div style={{ padding: '28px 32px', fontFamily: sg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <h1 style={{ fontFamily: sg, fontSize: 22, fontWeight: 700, color: '#1B1512', letterSpacing: '-0.03em', margin: 0 }}>
              Inscripciones
            </h1>
            {pendingCount > 0 && (
              <span style={{
                fontFamily: sg, fontSize: 12, fontWeight: 700, color: '#B45309',
                background: '#FFF7E0', borderRadius: 20, padding: '2px 10px',
              }}>
                {pendingCount} pendiente{pendingCount > 1 ? 's' : ''}
              </span>
            )}
          </div>
          <p style={{ fontFamily: sg, fontSize: 13, color: '#9a8f86', margin: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 220, position: 'relative' }}>
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#a89e95" strokeWidth="2" strokeLinecap="round"
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            style={{
              width: '100%', boxSizing: 'border-box', fontFamily: sg, fontSize: 13, color: '#1B1512',
              background: '#fff', border: '1.5px solid #E7DED6', borderRadius: 10,
        ...
        {(['all', 'pending', 'contacted', 'active'] as const).map((s) => {
          ...
          return (
            <button key={s} onClick={() => setFilterStatus(s)} style={{
              ...
              color: active ? '#fff' : '#5a4f47',
              background: active ? '#FF6A1A' : '#fff',
              border: `1.5px solid ${active ? '#FF6A1A' : '#E7DED6'}`,
      ...
      {filtered.length === 0 ? (
        <div style={{
          textAlign: 'center', padding: '64px 24px', color: '#a89e95',
          fontFamily: sg, fontSize: 14, background: '#fff', borderRadius: 14, border: '1px solid #F0E8E1',
        }}>
      ...
      {filtered.length > 0 && (
        <p style={{ fontFamily: sm, fontSize: 11, color: '#b5a99f', textAlign: 'right', marginTop: 16 }}>
